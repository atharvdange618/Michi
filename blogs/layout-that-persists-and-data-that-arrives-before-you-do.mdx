...
In the [previous post](https://tty.atharvdange.com/blog/what-happens-when-you-click-a-link-in-react), routes were a flat array:
...
The matching function from [the previous post](https://tty.atharvdange.com/blog/what-happens-when-you-click-a-link-in-react) used `matchRoute` on a flat array. The new version, `matchTree`, walks a tree and returns a flat array of matches:
...
In two slices, the router went from a flat list of independent pages to a tree of nested layouts with data loading. The core loop from the [previous post](https://tty.atharvdange.com/blog/what-happens-when-you-click-a-link-in-react) did not change, it just got richer. History notifies Router, Router matches routes, React renders components. The matching now walks a tree instead of a flat array. The rendering now uses a depth counter to nest components. The data now arrives before components mount.
