...
Every page renders in isolation. Navigate to `/about` and you get the About component. Navigate to `/user/atharv` and you get the User component. Nothing shared between them. The moment you want a navbar that stays alive across navigations, a sidebar that persists, or data that arrives before the component mounts, the flat architecture breaks.
...
A single line changed: `children`. The root layout wraps everything. When you navigate to `/about`, the router does not just find the About component. It finds the root layout AND the About component. Two matches, not one.
...
Here is what trips people up. The route definitions are a tree, but the matched result is a flat array. Why? Because React renders top to bottom, linearly. The tree structure is just for configuration. Once matching is done, you flatten it into an ordered list: root first, then the matched child. That flat array is what `<Outlet />` walks through.
...
For regular routes with children, the logic checks children first. If a child matches, the parent acts as a layout wrapper. If no child matches, the route falls through to its own leaf matching. A single route definition can serve as both a layout and a page, depending on whether its children match the URL.
...
Here is what makes it click. The `OutletContext` holds a single number: the current depth into `state.matches`. `RouterProvider` renders `matches[0]` directly and sets `OutletContext` to `1`. Each `<Outlet />` reads its index, renders that match, and increments for the next level.
...
This matters because of how layout routes work. Navigate to `/about` with this route tree:
...
Here is what makes this clever. `OutletContext` holds the index of the NEXT match to render. When `UserPage` is mounted by an `<Outlet />` at depth `1`, `OutletContext` inside `UserPage` is `2`, because Outlet set it to `matchIndex + 1` before rendering the component.
...
The problem with async loaders: you click Atharv, a 600ms fetch starts. Before it resolves, you click John, another 600ms fetch starts. If John's fetch resolves first, then Atharv's resolves, you would end up showing Atharv's data even though you are on John's URL.
...
Immediately showing a loading state on every navigation creates a problem. If a loader resolves in 50ms, the user never sees the loading state, but React still unmounts the old page, shows a spinner for one frame, then mounts the new page. That flash is jarring.
...
This ensures that once a spinner is shown, it stays on screen for at least 500ms, preventing a rapid, jarring flash if the loaders finish right after the timer fires.
...
<Callout type="info">
  **Threshold-based loading eliminates spinner flash.** A loading indicator that
  ...
  all. Delay the loading state by a threshold. If the data arrives before the
  threshold, the user sees a clean swap. If it is slow, they get a spinner, and
  it stays visible long enough to not feel like a glitch.
</Callout>
...
Three states. `state.status === "loading" && !rootMatch` is the initial load: no matches yet because loaders have not resolved, so a loading state is shown. `state.status === "error" && !rootMatch` handles loader errors. `rootMatch` existing means normal render. If none of these conditions match, it is a 404.
...
- [TanStack Router - Routing Concepts](https://tanstack.com/router/v1/docs/routing/routing-concepts): Layout Routes, Pathless Layouts, and the core tree hierarchy
- [React Router - Routing Guide](https://reactrouter.com/start/data/routing): Nested routing configuration, layout grouping, and Outlets

### Data Loaders

- [TanStack Router - Data Loading Guide](https://tanstack.com/router/v1/docs/guide/data-loading)
- [React Router - Data Loading](https://reactrouter.com/v7/start/framework/data-loading)
- [Remix docs - Data Loading](https://remix.run/docs/en/main/guides/data-loading)
