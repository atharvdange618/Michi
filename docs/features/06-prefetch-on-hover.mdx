...
`this.match(pathname, search)` is `matchTree` plus `applySearch` -- the exact functions that run on every real navigation. `runLoaders(...)` is the exact function from `loader.ts` that Slice 4 built and Slice 5 hardened. The only new code is the caching wrapper around it.
