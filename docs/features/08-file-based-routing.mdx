...
`fs.readdirSync` returns entries in whatever order the filesystem provides, and `$` sorts before letters in ASCII, so a dynamic route can land above a static sibling. Since Slice 10 the runtime matcher ranks routes by score, so this no longer changes which route matches. Codegen still sorts, so `routeTree.gen.ts` reads with specific routes above their dynamic siblings.

The sort uses four tiers checked in order, alphabetical within each tier:
