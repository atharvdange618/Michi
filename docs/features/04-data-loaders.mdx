...
With codegen, just export the function from your route file. The codegen detects it automatically via AST inspection -- no manual wiring needed. Run `pnpm codegen` after adding a new export.
...
If the route, its params, and its search params have not changed, the loader is skipped and the previous `loaderData` is reused. Navigate from `/user/atharv` to `/user/atharv` and zero network requests fire.
