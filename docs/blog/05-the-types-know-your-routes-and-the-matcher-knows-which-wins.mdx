...
The [previous post](/blog/the-url-remembers-and-the-filesystem-already-knows) ended with the route tree becoming a real source of truth. Slice 8's codegen reads the routes folder and writes `routeTree.gen.ts`: one directory in, one array out, nothing to keep in sync by hand. This post is about teaching the rest of the router to read that array properly.
